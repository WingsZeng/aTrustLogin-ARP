...
class ATrustLogin:
    ...
    def enter_credentials(self, username, password):
        try:
            element = self.driver.find_element(
                By.XPATH,
                "//div[contains(@class, 'server-name') and contains(text(), '本地密码')]",
            )
    ...
    def load_storage(self):
        ...
        try:
            if os.path.exists(os.path.join(self.data_dir, "ATrustLoginStorage.pkl")):
                with open(
                    os.path.join(self.data_dir, "ATrustLoginStorage.pkl"), "rb"
                ) as f:
                    data = pickle.load(f)
                    # 从cookies中加载cookie
                    ...
                    for key, value in data.local_storage.items():
                        ...
                        self.driver.execute_script(
                            f"window.localStorage.setItem({key_js}, {value_js});"
                        )
    ...
    def is_logged(self):
        ...
        return (
            "工作台" in self.driver.page_source
            and "本地密码" not in self.driver.page_source
        )
        ...
        while True:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                try:
                    ...
                    logger.info(
                        f"aTrust Port {port} is not yet being listened on. Waiting for aTrust start ..."
                    )
